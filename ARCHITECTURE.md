...
**Known exception:** the vault key does not follow this rule. The same key derived from Argon2id encrypts both `identity.enc` (custom AES-GCM) and the entire database (passed raw to SQLCipher via `PRAGMA key`). No dedicated sub-key for the two uses: a weakness in either encryption scheme exposes the other as well. Accepted in V1 to avoid an extra derivation step; if revisited, a per-use `HKDF(vault_key, info=...)` would be enough.
...
| Inbound | The sender's IP must match at least one saved contact's IP, otherwise the handshake fails immediately. If several contacts share the same IP, see the [limit described in the README](./README.md#what-v1-does-not-protect-against-by-design-not-by-oversight) |
...
Fixed 4096-byte block for every packet, regardless of type: each frame (nonce + ciphertext + tag) is padded up to that size before transport encryption. A message at the `MAX_MESSAGE_BYTES` limit (2500 bytes) comfortably stays under the threshold once encrypted and base64-encoded; there is no growing-bucket mechanism for larger content though — simplicity over overhead optimality.
...
- Symmetric ratchet (Signal-style KDF chain): one chain per direction, one-way advancement (HMAC-SHA256), a message key derived for every single message.
...
No backward compatibility between schema versions: project in demo stage, a format change requires recreating the database.
...
These values are calibrated for LAN/Tailscale (low latency). Should be revisited for V2 in the presence of a higher-latency transport.
